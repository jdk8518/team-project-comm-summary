...
def _string_items(data: dict, *keys: str) -> list[str]:
    for key in keys:
        value = data.get(key)
...
def _verification_issue_rows(issues: list[dict]) -> list[str]:
    return [
        f"| {issue['category']} | {issue['reason']} | "
        f"{'<br>'.join(issue['source_evidence'])} | {issue['human_check']} |"
        for issue in issues
    ]


def _review_items(analysis: dict, summary: dict, issues: list[dict]) -> list[str]:
    return [
        *_string_items(summary, "review_items"),
        *_string_items(analysis, "review_items"),
        *(issue["human_check"] for issue in issues),
    ]
...
def _markdown(result: dict, preview: dict) -> str:
    ...
    lines = [
        "# 문서 분석 결과", "", "## 1. 문서 기본 정보",
        f"- 문서 제목: {_first_text(analysis, 'title')}", f"- 문서 유형: {preview['document_type']}",
        f"- 기준 일자: {preview['reference_date']} ({document.get('reference_date_source', '확인 필요')})",
        f"- 작성 소속(기관): {_first_text(analysis, 'organization')}", f"- 부서: {_first_text(analysis, 'department')}",
        "", "## 2. 문서 구조와 핵심 내용",
        f"- 목차·본문 구조: {', '.join(_string_items(analysis, 'table_of_contents')) or '확인 필요'}",
        f"- 핵심 문장: {' / '.join(_string_items(analysis, 'key_sentences')) or '확인 필요'}",
        f"- 수치·기한: {', '.join(_string_items(analysis, 'numerical_values')) or '확인 필요'}",
        f"- 조건·예외: {', '.join(_string_items(analysis, 'conditions')) or '확인 필요'}",
        f"- 결정 사항: {', '.join(_string_items(analysis, 'decisions')) or '확인 필요'}",
        "", "## 3. 문서 핵심 요약",
        f"- 문서 개요: {_first_text(summary, 'document_overview', 'background')}",
        f"- 문서 목적·주요 내용: {_first_text(summary, 'document_purpose', 'main_content')}",
        f"- 결론 및 핵심 메시지: {_first_text(summary, 'conclusion_or_key_message', 'conclusion')}",
        f"- 주요 내용: {', '.join(_string_items(summary, 'main_contents', 'follow_up_actions')) or '확인 필요'}", "", "## 4. 주요 키워드",
        f"- 키워드: {', '.join(_string_items(analysis, 'keywords')) or '확인 필요'}", "", "## 5. 문서 검증 결과",
        f"- 검증 통과: {'예' if verification.get('passed') else '아니오'}",
        f"- 수정·확인 방향: {_first_text(verification, 'revision_instruction')}",
    ]
    issues = verification.get("issues", [])
    if issues:
        lines.extend(["", "| 검증 항목 | 문제가 되는 이유 | 관련 원문 근거 | 사람이 확인할 항목 |", "| --- | --- | --- | --- |"])
        lines.extend(_verification_issue_rows(issues))
    else:
        lines.append("- 확인이 필요한 검증 문제 없음")
    lines.extend(["", "## 6. 사람이 확인할 항목"])
    lines.extend(f"- {item}" for item in _review_items(analysis, summary, issues) or ["없음"])

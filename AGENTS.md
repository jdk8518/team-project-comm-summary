...
- 주요 기술: Language: Python 3
            Backend: FastAPI
            AI Workflow: OpenAI SDK 기반 AI provider adapter
            Document Processing: PDF, DOCX, TXT, HWP, HWPX, PPTX
            Frontend: 정적 HTML 기반 MVP UI
            Database: SQLite (`ARCHIVE_ROOT` 하위 `documents.db`)
- 요구사항 문서: `./docs/requirements-Gharam.md`
- 기능 분해 문서: `./docs/function-breakdown-Gharam.md`
- MVP 계획 문서: `./docs/mvp-plan-Gharam.md`

## 2. 작업 시작 순서

1. `./AGENTS.md`를 읽는다.
2. `./README.md`를 읽는다.
3. `./docs/requirements-Gharam.md`를 읽는다.
4. `./docs/function-breakdown-Gharam.md`를 읽는다.
5. `./docs/mvp-plan-Gharam.md`를 읽는다.
...
`HwpxDocument.open()`으로 HWPX 원본을 엽니다. HWPX를 수정·생성하는 기능은 예를 들어 `output/report-updated.hwpx`처럼 새 경로에 저장하며, 개발·테스트 중 사용자 원본을 덮어쓰지 않습니다. HWP(v5 바이너리)의 텍스트 추출은 `docpler`를 사용하고, HWPX는 `python-hwpx`를 사용합니다.

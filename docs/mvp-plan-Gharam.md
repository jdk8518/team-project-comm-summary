...
### 1.2. MVP 한 줄 설명
사용자가 단일 문서를 업로드하여 AI 분석·3단 요약·검증 결과를 확인하고, DB에 저장된 문서 이력 리스트를 검색·선택하여 요약 내용 조회 및 원본 파일 1-Click 다운로드를 수행할 수 있는 최소 실행 버전
...
flowchart TD
    ...
    subgraph Flow1 ["1. 문서 업로드 & AI 분석·검증 파이프라인"]
        ...
        C_Check -- "추출 텍스트 20자 미만 / 스캔 이미지" --> E3[에러: 텍스트 미존재 및 OCR 필요 알림]:::error
        E3 --> A
...
|:---:|---|---|---|
| **1** | **문서 입력 및 유효성 검증 (`FEAT-01`)** | 단일 문서 파일(PDF, DOCX, TXT, HWP, HWPX, PPTX / 최대 50MB) 수신, 3단계(형식·크기·내용 손상) 유효성 검증 및 예외 알림 처리 | 사용자가 문서를 입력하고 유효하지 않은 파일 입력을 사전 차단 및 안내받는 데 필수적임 |
| **2** | **문서 텍스트 추출 및 정제 (`FEAT-02`)** | 6종 문서 포맷별 전용 파서를 통한 Raw 텍스트 파싱, 불필요한 공백/줄바꿈 정제 및 기본 구조 태깅 | AI 분석 및 요약 파이프라인의 입력 텍스트 데이터 생성을 위해 문서 핵심 처리에 필수적임 |
...
|:---:|---|---|---|
| **1** | **스캔본 PDF/HWP OCR 텍스트 인식** | MVP 범위는 디지털 텍스트 레이어가 포함된 문서 처리에 집중하며, OCR 인식률 오류 보정 및 외부 엔진 연동의 복잡성을 배제하기 위함 | 디지털 텍스트 문서 파이프라인(파싱-분석-요약-검증)이 안정화되고, Tesseract 또는 외부 OCR 엔진 연동 환경이 구축되면 추가 |
...
| **PDF 처리 범위** | 텍스트 레이어가 포함된 정상 PDF (스캔 이미지 전용 제외) | OCR 연동 복잡성을 배제하고 디지털 텍스트 추출 및 AI 분석 파이프라인 검증에 집중하기 위함 |
...
| **HWP 처리 범위** | HWP (v5 바이너리) 문서는 파싱 전 HWPX 변환 파이프라인 사전 적용 | `python-hwpx` 호환성 유지 및 HWP 파싱 실패율을 최소화하기 위함 |
...
| **DB 처리방법** | SQLite, PostgreSQL, MySQL 등 적용할 DB 엔진 및 저장 스키마 규격 | DB 연결 모듈, ORM 도입 여부 및 요약 검색 API 구현 방식에 영향을 줌 |
...
| **4. 수업 시간 안에 구현 가능한 범위인가** | **통과** | OCR 파싱, 다중 문서 비교 분석, 외부 연동 등 고난도/외곽 기능을 후순위로 제외하고 핵심 7개 기능으로 범위를 한정함 | 해당 없음 (현재 계획 유지) |
...
| **7. 포함 기능과 제외 기능이 명확한가** | **통과** | FEAT-01~FEAT-07은 MVP 포함으로, OCR/비교분석/PDF변환 등 6개 기능은 사유 및 추가 조건과 함께 명확히 분리됨 | 해당 없음 (현재 계획 유지) |
